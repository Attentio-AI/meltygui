...
"""Two peer OS windows, one with a child window that follows it.
    ...
    PYTHONPATH=<latent-descent> python meltygui/examples/two_windows.py
"""
import meltygui
...
def notes_window():
    global notes, log
    changed, new = meltygui.draw_text(notes, name='notes', syntax_highlight=False)
    if changed:
        notes = new
    # A child OS window of this one: onable, follows it, closes
    # when this call stops (Ctrl+L toggles it).
    if not state['hide_log']:
        changed, new = meltygui.draw_text(log, name='log', glfw_window=True, window_size=(520, 300),
...
def scratch_window():
    meltygui.draw_text("a second peer window\n", name='scratch', syntax_highlight=False)
