...
class DynamicObj(DictConversion):
    ...
    def __getattribute__(self, name):
        ...
        if self.driver is not None and self.root is not None:
            driver = self.root.get(self.driver)
        else:
            driver = {}
        ...
        if name != 'driver' and name != 'root' and name in driver:
            # Create the attribute with None value
            setattr(self, name, None)
            return None
