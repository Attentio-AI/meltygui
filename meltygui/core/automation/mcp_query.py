...
def _subscriptions_for(tile_id):
    ...
    if handler is None:
        return []
    import meltygui.core.input.input_handler as IH
    subs = []
...
def collect_param_sources(view, param=""):
    ...
    if ds is None:
        return {"frame": Melty.frame_count, "error": error}
    import meltygui.core.rendering.parameter_core as A
...
def run_query(collect, model_server, timeout=10.0):
    ...
    from meltygui.core.automation.mcp_eval import request_call
