# Adapted from pyimgui 2.0.0 imgui.integrations.glfw (BSD-3-Clause).
# Only the window API differs; renderer subclasses keep their existing overrides.
# -*- coding: utf-8 -*-
...
class WindowRenderer(ProgrammablePipelineRenderer):
    ...
    def keyboard_callback(self, window, key, scancode, action, mods):
        ...
        if key < 0 or key >= len(io.keys_down):
            return
    ...
    def char_callback(self, window, char):
        # Dispatch may happen while another Surface's ImGui context is active.
        io = self.io
